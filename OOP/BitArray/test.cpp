...
TEST(BitArrayTest, ConstructorWithValueEx) {
    EXPECT_THROW(BitArray bits(-1, 8), length_error);
}

TEST(BitArrayTest, SetWithArgsTestEx) {
    BitArray bits(8, 4);
    EXPECT_THROW(bits.set(-1, 1), out_of_range);
    EXPECT_THROW(bits.set(100, 1), out_of_range);
}

TEST(BitArrayTest, ResetWithArgsTestEx) {
    BitArray bits(8, 4);
    EXPECT_THROW(bits.reset(-1), out_of_range);
    EXPECT_THROW(bits.reset(100), out_of_range);
}

TEST(BitArrayTest, ResizeWithArgsTestEx) {
    BitArray bits(8, 4);
    EXPECT_THROW(bits.resize(-1), invalid_argument);
}

TEST(BitArrayTest, BitAndTestEx) {
    BitArray bits(8, 4);
    BitArray bits2(12, 4);
    EXPECT_THROW(bits&=bits2, length_error);
}

TEST(BitArrayTest, BitOrTestEx) {
    BitArray bits(8, 4);
    BitArray bits2(12, 4);
    EXPECT_THROW(bits|=bits2, length_error);
}

TEST(BitArrayTest, BitwiseOrTestEx) {
    BitArray bits(8, 4);
    BitArray bits2(12, 4);
    EXPECT_THROW(bits^=bits2, length_error);
}

TEST(BitArrayTest, IndexoperatorEx) {
    BitArray bits(8, 4);
    EXPECT_THROW(bits[-1], out_of_range);
    EXPECT_THROW(bits[100], out_of_range);
}